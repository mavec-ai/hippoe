...
async fn demo_fastembed() {
    ...
    match provider.embed_batch(&texts).await {
        Ok(embeddings) => {
            ...
            let matches: Vec<RetrievalMatch> = hippoe
                .query()
...
async fn demo_ollama() {
    ...
    let texts = [
        ...
        (
            "Artificial intelligence and deep learning are revolutionizing tech",
            0.6,
        ),
        (
    ...
    match provider.embed_batch(&text_refs).await {
        Ok(embeddings) => {
            ...
            for (i, ((text, importance), embedding)) in
                texts.iter().zip(embeddings.iter()).enumerate()
            {
                let memory = MemoryBuilder::new(embedding.clone(), 1_000_000_000 + i as u64)
                    .text(text.to_string())
