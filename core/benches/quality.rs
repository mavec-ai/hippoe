...
use hippoe_core::{
    AssociationGraph, HippocampusBuilder, Embedding, Memory,
    MemoryBuilder, RetrievalContext, RetrievalMatch, RetrievalStrategy, Timestamp,
    InMemoryStorage, similarity_batch,
};
...
fn get_test_cases() -> Vec<TestCase> {
    ...
    vec![
        ...
        TestCase {
            ...
            description: "Recently accessed items in working memory should surface.",
            probe: make_embedding(&[1.0, 0.0, 0.0], dim),
            current_time: now,
            memories: vec![
                TestMemory {
        ...
        TestCase {
            name: "fan_effect",
            description: "Memory with fewer associations spreads stronger activation.",
            probe: make_embedding(&[1.0, 0.0, 0.0], dim),
            current_time: now,
            memories: vec![
                TestMemory {
            ...
            associations: vec![
                TestAssociation { source_idx: 2, target_idx: 0, strength: 0.9 },
            ],
        },
        TestCase {
            ...
            associations: vec![
                TestAssociation { source_idx: 0, target_idx: 1, strength: 0.9 },
                TestAssociation { source_idx: 1, target_idx: 2, strength: 0.8 },
            ],
        },
    ]
}
...
async fn run_cognitive_retrieval(test: &TestCase, memories: &[Memory]) -> Vec<usize> {
    ...
    for memory in memories.iter() {
        let id = memory.id;
        hippoe.memorize(memory.clone()).await.expect("Failed to memorize");
        memory_ids.push(id);
    }
    ...
    for assoc in &test.associations {
        ...
        hippoe.create_association(from, to, assoc.strength, hippoe_core::LinkKind::Semantic)
            .await
...
fn main() {
    ...
    for test in &test_cases {
        ...
        println!("  Naive:     NDCG={:.4}, MRR={:.4}, P@5={:.4}", 
            naive_metrics.ndcg, naive_metrics.mrr, naive_metrics.precision_at_k);
        println!("  Cognitive: NDCG={:.4}, MRR={:.4}, P@5={:.4}", 
